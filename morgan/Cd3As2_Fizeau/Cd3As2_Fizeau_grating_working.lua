...
for i = 1, #theta do
    for freq=0.01,0.1000,0.0001 do
        S:SetExcitationPlanewave(
        {theta[i], 90}, -- incidence angles -- phi in [0,180), theta in [0,360)
        ...
        q = (omega/c)*math.sin(theta_q[i]*(math.pi/180))
        ...
        -- B = ((Omega_d + (I/Tau)) / (Q_d*2))*(log_real_b+log_imag_b) - 1
        -- sigma_d = (gamma^(-2/3))*( Omega/Omega_d)*((I * g * eV^2 * k_F) / (2 * math.pi^2 * hbar)) * (Omega_d / (Q_d^2)) * (((Omega_d + (I / Tau)) * B) / (Omega_d - (I / Tau) * B))
        -- eps_d0 = eps_inf + (I*(sigma_d*hbar) / (eps_0*E))    -- NO current bias
