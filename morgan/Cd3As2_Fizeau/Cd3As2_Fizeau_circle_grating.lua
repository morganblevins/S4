...
-- Using base 1 micron for units

-- Using grating design from "Axion-Field-Enabled Nonreciprocal Thermal Radiation in Weyl Semimetals"
-- by Bo Zhao et al

lat = 12
S:SetLattice({lat,0}, {0,lat})
S:SetNumG(10)

S:AddMaterial("Cd3As2", {1,0})
S:AddMaterial('Vacuum', {1,0})
S:AddMaterial('SiO2', {4,0})
S:AddMaterial("PerfMirror", {-1.0e10,0})

S:AddLayer('Above', 0 , 'Vacuum')
-- S:AddLayer('Dummy', 1 , 'Vacuum')
S:AddLayer('Slab', 0.5, 'Cd3As2')
-- S:AddLayer('Slab', 0.1, 'Vacuum')
-- S:AddLayer('Base', 0.25*lat, "Cd3As2")
S:AddLayer('Below', 0, 'Cd3As2') -- layer to copy
-- S:AddLayer('Below', 0, 'PerfMirror') -- layer to copy

S:SetLayerPatternCircle('Slab',   -- layer to pattern
                        'Vacuum', -- material inside circle
                        {0,0},    -- center of circle
                        lat*0.29)      -- radius of circle
...
S:SetLayerPatternCircle('Slab',   -- layer to pattern
                        ...
                        lat*0.29)      -- radius of circle
-- S:SetLayerPatternCircle('Slab',   -- layer to pattern
--                         'Cd3As2', -- material inside circle
--                         {0,0},    -- center of circle
--                         0.4)      -- radius of circle

-- S:SetLayerPatternRectangle('Slab', 'Vacuum', {0,0}, 0, {1*lat,0.5*lat})
                        
ang_inc = 80
local theta = {ang_inc, 180-ang_inc}
local theta_q = {ang_inc, -ang_inc}
ang_rot = 180

for freq_eV=0.044,0.060,0.000001 do
-- for freq_eV=0.045,0.08,0.00001 do
    S:SetExcitationPlanewave(
    {theta[1], ang_rot}, -- incidence angles -- phi in [0,180), theta in [0,360)
    {0,0}, -- s-polarization amplitude and phase (in degrees)
    {1,0}) -- p-polarization amplitude and phase
    -- Define frequency
    f = 0.8065548889615557*freq_eV -- using units of [eV]
    E = freq_eV*eV -- [J]
    E_E_F = E / E_F --[nondimensionallized in E_F]
    omega = E/hbar --[1/s]
    
    q = (omega/c)*math.sin(theta_q[1]*(math.pi/180))
    ...
    -- with current bias   
    vd = vFactor*v_F
    gamma   = math.sqrt(1 - (vd^2 / v_F^2))^(-1) -- QLT transform parameter
    ...
    sig_dr = complex.real(sigma_d)
    sig_di = complex.imag(sigma_d)

    -- print('eps_d0 = '.. eps_d0 .. ', eps_d = ' .. eps_d)
    ...
    -- Reflected power
    forward1,reflected1 = S:GetPoyntingFlux('Above', 0)
    reflected1 = -reflected1/forward1
    -- T1x,T1y,T1z = S:GetStressTensorIntegral('Above', 1)
    -- T2x,T2y,T2z = S:GetStressTensorIntegral('Below', 0)

    S:SetExcitationPlanewave(
    {theta[2], ang_rot}, -- incidence angles -- phi in [0,180), theta in [0,360)
    {0,0}, -- s-polarization amplitude and phase (in degrees)
    {1,0}) -- p-polarization amplitude and phase
    -- Define frequency

    q = (omega/c)*math.sin(theta_q[2]*(math.pi/180))
    Q_k_F = q / k_F -- Dimensionless wave vector [1]. 
    ...
    -- Reflected power
    forward2,reflected2 = S:GetPoyntingFlux('Above', 0)
    reflected2 = -reflected2/forward2
